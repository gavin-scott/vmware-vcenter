...
def collect_inventory(obj, parent=nil)
  ...
  case obj
    ...
    when RbVmomi::VIM::VmwareDistributedVirtualSwitch
       ...
       hash[:attributes]["hosts"] = obj.FetchDVPorts!.map {|d| d.proxyHost.name if d.proxyHost}.compact.uniq rescue STDERR.puts "WARNING: FetchDVPorts! on %s failed: %s: %s" % [obj.name, $!.class, $!.to_s]
       hash[:attributes]["hosts"] ||= []
